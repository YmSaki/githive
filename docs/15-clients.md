...
- **Claude plugin 形式**：plugin マニフェスト + MCP サーバー定義 + スキル（githive の使い方を教える SKILL.md）を同梱したパッケージを配布する。スキルには「clone 直後に `githive init` と `githive status` を実行して文脈を得る」等の運用知識を書く。
...
- githive 本体とはプロセス分離し、`githive --json` を子プロセスとして呼ぶ。Go 側の API 安定性は JSON 封筒（[10](10-cli-spec.md)）が担う。
  MCP ではなく CLI を使うのは、拡張が必要とする操作粒度が CLI と同じで、MCP のセッション管理を挟む利点がないためである。
- 機能：
...
MCP を TUI より先にするのは、Agent が clone 直後から使えることがこのシステムの中核価値であり、TUI は人間の快適さの改善で代替手段（CLI）があるためである。
