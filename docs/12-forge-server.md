...
1. **名前空間**：ref 名を policy（users 台帳）で評価し、どのルールにも一致しない ref 更新は既定 deny で拒否（push をブロック）する。組み込みの既定ルールは `refs/heads/**`、`refs/tags/**`、`refs/projects/**` を許可する。`refs/notes/**` 等の正当な git 名前空間を使うプロジェクトは policy にルールを足して許可する。未知の名前空間を「検証なしで素通し」にはしない。権限の定義が無い場所への書き込みは無認可のデータ置き場になるためである。
2. **権限**：許可された名前空間について、policy で actor と action（push/create/delete/force）を評価。
...
- 定義はリポジトリ内 `.githive/workflows/*.yml`。トリガは `push`（ブランチ）、`projects`（githive イベント。例 `issue.status == resolved`）、`schedule`。
