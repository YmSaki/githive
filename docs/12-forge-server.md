...
1. **名前空間**：`refs/heads/*`、`refs/tags/*`、`refs/projects/*` 以外を拒否。
2. **権限**：users 台帳（その時点の registry）の policy で actor と action（push/create/delete/force）を評価。
3. **署名**：`refs/projects/*` への push は全コミットに有効署名を要求。actor と鍵の一致（[11](11-security.md) の検証規則）を確認。
...
書き込み UI は CLI/TUI/VSCode 拡張が先にあるため優先度を下げる。
